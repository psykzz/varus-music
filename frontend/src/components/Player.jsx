...
export default function Player({
  ...
  return (
    <>
      ...
      <div className="hidden md:flex flex-col px-6 py-3 gap-2 bg-spotify-darkgray/90">
        {/* Progress bar */}
        <div className="flex items-center gap-2 text-xs text-spotify-lightgray">
          <span>{formatTime(currentTime)}</span>
          <div
            className="flex-1 h-1.5 bg-spotify-gray/70 rounded-full cursor-pointer relative group"
            onClick={handleSeek}
          >
            <div
              className="h-full bg-spotify-green rounded-full group-hover:bg-cyan-100 transition-colors"
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              onClick={togglePlay}
              className="w-11 h-11 sonic-primary rounded-full flex items-center justify-center hover:scale-105 transition-transform"
              aria-label={isPlaying ? "Pause" : "Play"}
              style={{ boxShadow: "0 0 16px rgba(0,244,254,0.3)" }}
            >
      ...
      <div
        className="md:hidden fixed left-0 right-0 z-40 sonic-glass cursor-pointer touch-manipulation"
        style={{
      ...
      >
        ...
        <div className="absolute top-0 left-0 right-0 h-0.5 bg-spotify-gray/90">
          <div
        ...
        <div className="flex items-center gap-3 px-4 h-full">
          {/* Album art thumbnail */}
          <div className="w-10 h-10 rounded overflow-hidden bg-spotify-darkgray/95 border border-spotify-lightgray/15 flex-shrink-0">
            {track.albumArtUrl ? (
              <img
              ...
              <span className="text-lg w-full h-full flex items-center justify-center select-none">
                🎵
              </span>
            )}
          </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-spotify-lightgray truncate leading-tight">
              {track.artist}
            </p>
          </div>
          ...
          <button
            ...
            className="w-11 h-11 sonic-primary rounded-full flex items-center justify-center flex-shrink-0 touch-manipulation hover:scale-105 transition-transform"
            aria-label={isPlaying ? "Pause" : "Play"}
          >
            {isPlaying ? <PauseIcon /> : <PlayIcon />}
          </button>
      ...
      {mobileExpanded && (
        <div className="md:hidden fixed inset-0 z-50 bg-[radial-gradient(circle_at_20%_5%,rgba(161,250,255,0.16),transparent_40%),linear-gradient(180deg,#19191f,#0e0e13)] overflow-hidden flex flex-col landscape:flex-row">
          {/* ── Portrait: Dismiss header ── */}
          <div
            className="flex items-center justify-between px-5 pt-safe landscape:hidden"
          ...
          >
            ...
            <span className="text-xs font-semibold uppercase tracking-widest text-spotify-lightgray">
              Now Playing
            </span>
            <div className="w-9" />
          </div>
          ...
          <div className="hidden landscape:flex landscape:flex-col landscape:w-[45%] landscape:items-center landscape:justify-center landscape:px-6 landscape:py-4 landscape:pt-safe landscape:gap-3 flex-shrink-0">
            ...
            <div className="w-full max-w-[200px] aspect-square rounded-xl shadow-2xl overflow-hidden bg-spotify-gray/90 border border-spotify-lightgray/15 flex items-center justify-center">
              {track.albumArtUrl ? (
                <img
                  src={track.albumArtUrl}
          ...
          <div className="flex landscape:hidden items-center justify-center px-8 py-2 flex-shrink-0">
            <div className="w-full max-w-[300px] aspect-square rounded-2xl shadow-2xl overflow-hidden bg-spotify-gray/90 border border-spotify-lightgray/15 flex items-center justify-center">
              {track.albumArtUrl ? (
                <img
                  src={track.albumArtUrl}
                  alt={track.album || track.title}
          ...
          <div className="flex flex-col flex-1 min-h-0 overflow-y-auto px-6 py-4 landscape:justify-center landscape:pr-6 landscape:pt-safe">
            ...
            <div className="flex items-start justify-between gap-4 mb-4">
              <div className="min-w-0">
                <h2 className="text-xl font-heading font-bold text-white truncate landscape:text-lg tracking-tight">
                  {track.title}
                </h2>
                <p className="text-spotify-lightgray truncate">
                  {track.artist}
                </p>
                {track.album && (
            ...
            <div className="flex items-center gap-2 text-xs text-spotify-lightgray mb-4">
              ...
              <div
                className="flex-1 h-2 bg-spotify-gray/90 rounded-full cursor-pointer relative group"
                onClick={handleSeek}
              >
                <div
                  className="h-full bg-spotify-green rounded-full group-hover:bg-cyan-100 transition-colors"
            ...
            <div className="flex items-center justify-between mb-5">
              <button
                onClick={onToggleShuffle}
                className={`p-3 touch-manipulation transition-colors rounded-xl ${shuffle ? "text-spotify-green bg-cyan-400/10" : "text-spotify-lightgray hover:text-white hover:bg-white/6"}`}
                aria-label={shuffle ? "Shuffle on" : "Shuffle off"}
              >
                <ShuffleIcon />
              </button>
              ...
              <button
                onClick={togglePlay}
                className="w-16 h-16 sonic-primary rounded-full flex items-center justify-center hover:scale-105 transition-transform touch-manipulation landscape:w-12 landscape:h-12 shadow-[0_0_24px_rgba(0,244,254,0.35)]"
                aria-label={isPlaying ? "Pause" : "Play"}
              >
              ...
              <button
                onClick={onToggleLoop}
                className={`p-3 touch-manipulation transition-colors rounded-xl ${loop ? "text-spotify-green bg-cyan-400/10" : "text-spotify-lightgray hover:text-white hover:bg-white/6"}`}
                aria-label={loop ? "Loop on" : "Loop off"}
              >
                <LoopIcon />
              </button>
...
function VolumeIcon() {
  return (
    <svg
      className="w-4 h-4 text-spotify-lightgray"
      fill="currentColor"
      viewBox="0 0 24 24"
    >
