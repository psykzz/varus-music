    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-[radial-gradient(circle_at_20%_0%,rgba(161,250,255,0.1),transparent_50%),linear-gradient(180deg,#0e0e13,#0e0e13)]">
      ...
      <div className="flex items-center justify-between px-6 py-4 sonic-glass shrink-0">
        <div>
          ...
          <p className="text-xs text-spotify-lightgray mt-0.5">
            How your playlist is generated
          </p>
        </div>
        <button
          onClick={onClose}
      ...
      {data && !loading && (
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          {/* Tabs */}
          <div className="flex border-b border-spotify-gray/80 bg-spotify-darkgray/90 shrink-0">
            {TABS.map((tab) => (
              <button
                key={tab.id}
              ...
              >
                {tab.label}
                {tab.id === "popular" && (
                  <span className="ml-1.5 text-xs text-spotify-lightgray">
                    ({data.summary.totalTracks})
                  </span>
                )}
                {tab.id === "rated" && (
                  <span className="ml-1.5 text-xs text-spotify-lightgray">
                    ({data.summary.ratedCount})
                  </span>
                )}
                {tab.id === "unheard" && (
                  <span className="ml-1.5 text-xs text-spotify-lightgray">
                    ({data.summary.unheardCount})
                  </span>
                )}
                {activeTab === tab.id && (
          ...
          <div className="flex-1 overflow-y-auto">
            {activeTab === "popular" && (
              <PopularList tracks={data.popularTracks} />
            )}
            {activeTab === "rated" && <RatedList tracks={data.ratedTracks} />}
            {activeTab === "unheard" && (
              <UnheardList tracks={data.unheardTracks} />
            )}
          </div>
        </div>
      )}
...
function AlgorithmBanner({ algorithm, summary }) {
  ...
  const progress = Math.min(
    100,
    Math.round(
      (algorithm.userTotalRatings / algorithm.discoveryThreshold) * 100,
    ),
  );

  return (
    <div
      className={`px-6 py-4 border-b border-spotify-gray shrink-0 ${isDiscovery ? "bg-blue-950/40" : "bg-green-950/40"}`}
    >
...
function PopularList({ tracks }) {
  ...
  return (
    <table className="w-full text-sm">
      <thead className="sticky top-0 bg-spotify-darkgray text-spotify-lightgray text-xs uppercase tracking-[0.14em]">
        <tr>
      ...
      <tbody>
        {tracks.map((track, i) => {
          const barWidth =
            maxScore > 0
              ? Math.round((track.popularityScore / maxScore) * 100)
              : 0;
          return (
            <tr
              key={track.id}
              className="border-t border-spotify-gray/80 hover:bg-spotify-gray/35 transition-colors"
            >
              ...
              <td className="px-4 py-3">
                ...
                <div className="text-spotify-lightgray text-xs truncate">
                  {track.artist}
...
function RatedList({ tracks }) {
  if (tracks.length === 0) {
    return (
      <EmptyMessage>
        You haven't rated any tracks yet. Like or dislike tracks while listening
        to get started.
      </EmptyMessage>
    );
  }
...
function UnheardList({ tracks }) {
  if (tracks.length === 0) {
    return (
      <EmptyMessage>
        You've rated every track in your library — impressive!
      </EmptyMessage>
    );
  }
...
function EmptyMessage({ children }) {
  return (
    <div className="flex items-center justify-center h-48">
      <p className="text-spotify-lightgray text-sm text-center max-w-sm px-4">
        {children}
      </p>
    </div>
  );
