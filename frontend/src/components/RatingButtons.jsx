...
export default function RatingButtons({ track, onRatingUpdate, size = "md" }) {
  ...
  // Reset the displayed rating whenever the track changes
  useEffect(() => {
    setLastRating(null);
  }, [track?.id]);
    ...
    <div className="flex items-center gap-0.5">
      <button
        ...
        className={`${buttonClass} rounded-lg transition-colors ${
          lastRating === 1
            ? "text-spotify-green bg-cyan-400/10"
            : "text-spotify-lightgray hover:text-spotify-green hover:bg-spotify-gray/70"
        } disabled:opacity-50`}
        aria-label="Like"
        title="Like"
      >
      ...
      <button
        ...
        className={`${buttonClass} rounded-lg transition-colors ${
          lastRating === -1
            ...
            : "text-spotify-lightgray hover:text-red-300 hover:bg-spotify-gray/70"
        } disabled:opacity-50`}
        aria-label="Dislike"
        title="Dislike"
      >
